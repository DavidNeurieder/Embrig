...
use openhil_core::fault::{Fault, FaultRule};
...
use openhil_models::{build_simulation_indexed, VehicleConfig};
use thiserror::Error;
...
impl VirtualTarget {
    ...
    pub fn new(config: &VehicleConfig, dbc_path: &Path) -> Result<Self, TargetError> {
        ...
        let network =
            openhil_dbc::parse(&text).map_err(|e| TargetError::Can(format!("invalid DBC: {e}")))?;
...
impl HardwareTarget {
    pub fn new(interface: &str, network: Network) -> Result<Self, TargetError> {
        let bus = openhil_can::SocketCanBus::open(interface)
            .map_err(|e| TargetError::Can(e.to_string()))?;
        Ok(Self { bus, network })
    }
}
