...
use openhil_core::time::{ms, Timestamp};
...
impl Ecu for VehicleController {
    ...
    fn set_signal(
        ...
        _id: u32,
        _signal: &str,
        _value: openhil_core::signal::SignalValue,
    ) -> Result<(), EcuError> {
...
mod tests {
    ...
    use openhil_core::simulation::Simulation;
    use openhil_core::time::US_PER_MS;
    use openhil_core::SignalValue;
    ...
    fn powertrain() -> (Simulation, usize, usize, usize, usize) {
        let network = openhil_dbc::parse(TESTS_DBC).unwrap();
    ...
    fn lost_battery_message_times_out_motor() {
        ...
        sim.add_fault(openhil_core::fault::FaultRule {
            fault: openhil_core::fault::Fault::DropFrame { id: ID_BATTERY },
            start: ms(200),
            duration: None,
        });
        sim.run_ms(1000);
    ...
    fn charger_faults_when_battery_stale() {
        ...
        let req =
            openhil_core::frame::CanFrame::new(ID_CHARGE_REQUEST, vec![1, 0, 0, 0, 0, 0, 0, 0])
                .unwrap();
        sim.inject(req);
        sim.run_ms(100);
        assert_eq!(
