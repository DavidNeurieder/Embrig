...
pub enum ModelError {
    ...
    ParseDbc(openhil_dbc::ParseError),
...
pub fn build_simulation_indexed(
    ...
    let network = openhil_dbc::parse(&text).map_err(ModelError::ParseDbc)?;
...
mod tests {
    ...
    fn write_temp(text: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("openhil-model-{}", std::process::id()));
...
"#,
    ...
    fn builds_and_runs_powertrain() {
        ...
        let enable = openhil_dbc::parse(DBC)
            .unwrap()
            .message(ecus::ID_MOTOR_ENABLE)
            .unwrap()
